...
雲端基礎結構上的Adobe Commerce支援與整合 [AWS PrivateLink](https://aws.amazon.com/privatelink/) 或 [Azure私人連結](https://learn.microsoft.com/en-us/azure/private-link/) 服務。 Adobe Commerce您可以使用PrivateLink在雲端基礎結構環境與託管於外部系統的服務和應用程式之間建立安全的、私人的通訊。 必須透過在同一雲端區域內，相同雲端平台(AWS或Azure)上設定的虛擬私人雲端(VPC)端點，才能存取Adobe Commerce應用程式和外部系統。

>[!TIP]
>
>PrivateLink最適合用於保護非HTTP整合的連線，例如資料庫或檔案傳輸。 如果您打算將應用程式與Adobe Commerce API整合，請參閱如何建立 [AdobeAPI網格](https://developer.adobe.com/graphql-mesh-gateway/gateway/create-mesh/) 在 _Adobe Developer App Builder的API網格_.
...
- 您無法使用PrivateLink建立SSH連線。 另請參閱 [啟用SSH金鑰](secure-connections.md).
- Adobe Commerce支援不包含疑難排解AWS PrivateLink初始啟用以外的問題。
- 客戶需自行負責與管理其VPC相關的成本。
- 您無法透過Azure私人連結使用HTTPS通訊協定（連線埠443）連線至雲端基礎結構上的Adobe Commerce，原因如下 [Fastly來源遮罩](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/faq/fastly-origin-cloaking-enablement-faq.html). 此限制不適用於AWS PrivateLink。
- 無法使用PrivateDNS。
...
- **雙向PrivateLink** — 選擇此設定，在雲端基礎結構環境上，建立與Adobe Commerce外部系統的安全連線，以及建立與系統的安全連線。 雙向選項需要兩個連線：
...
>[!WARNING]
>
>啟用PrivateLink最多需要 _五_ 工作日。 提供不完整或不準確的資訊可能會延遲處理程式。

### 必要條件

![check](../../assets/fix.svg) 雲端帳戶(AWS或Azure)，與雲端基礎結構執行個體上的Adobe Commerce位於相同區域。

![check](../../assets/fix.svg) 客戶環境中的VPC，裝載服務以透過PrivateLink連線。 請參閱AWS或Azure檔案，以取得VPC設定的說明，或連絡您的網路管理員。

![check](../../assets/fix.svg) 對於雙向PrivateLink連線，您必須先建立應用程式或服務的端點服務組態，並在VPC環境中建立端點，才能要求PrivateLink啟用。 另請參閱 [設定雙向PrivateLink連線](#set-up-for-bidirectional-privatelink-connections).

收集啟用PrivateLink所需的下列資料：

- **Customer Cloud帳戶號碼** (AWS或Azure) — 必須在雲端基礎結構例項上與Adobe Commerce相同的區域
- **雲端區域** — 提供託管帳戶的雲端區域以進行驗證
- **服務和通訊連線埠**—Adobe必須開啟連線埠，才能啟用VPC之間的服務通訊，例如SQL連線埠3306、SFTP連線埠2222
- **專案ID** — 提供雲端基礎結構上的Adobe Commerce Pro專案ID。 您可以使用以下專案取得專案ID和其他專案資訊 [雲端CLI](../dev-tools/cloud-cli-overview.md) 命令： `magento-cloud project:info`
- **連線型別** — 指定連線對型別的單向或雙向
- **端點服務** — 如果是雙向PrivateLink連線，請提供Adobe必須連線的VPC端點服務的DNS URL，例如： `com.amazonaws.vpce.<cloud-region>.vpce-svc-<service-id>`
- **已授予端點服務存取權** — 若要連線到外部服務，請允許端點服務存取下列AWS帳戶主體： `arn:aws:iam::402592597372:root`

  >[!WARNING]
  >
  >如果未提供端點服務的存取權，則VPC中服務的雙向PrivateLink連線為 **非** 新增，這會延遲設定。

#### 啟用Azure私人連結專用的其他先決條件

- 提供叢集ID；使用SSH登入遠端並使用命令： `cat /etc/platform_cluster`
...
1. **客戶** 透過主旨列提交請求PrivateLink啟用的支援票證 `PrivateLink support for <company>`. 包含 [啟用所需的資料](#prerequisites) 在票證中。 Adobe使用支援票證在啟用流程中協調通訊。
...
1. **客戶** 新增Adobe端點服務至其雲端帳戶(AWS或Azure)，這會觸發連線要求以Adobe。 如需指示，請參閱Cloud平台檔案：

   - 若為AWS，請參閱 [接受和拒絕介面端點連線要求].
   - 若為Azure，請參閱 [管理連線要求].

1. **Adobe** 核准連線要求。

1. 連線要求核准後， **客戶** [驗證連線](#test-vpc-endpoint-service-connection) 在VPC與AdobeVPC之間。

1. 啟用雙向連線的其他步驟：

   - **Adobe** 提供Adobe帳戶主體(AWS或Azure帳戶的根使用者)並請求對客戶VPC端點服務的存取權。
   - **客戶** 可讓Adobe存取客戶VPC中的端點服務。 這假設Adobe帳戶主體有權存取 `arn:aws:iam::402592597372:root`，如先前所述 **已授予端點服務存取權** 先決條件。

      - 更新客戶端點服務設定，以接受從Adobe帳戶起始的請求。 如需指示，請參閱Cloud平台檔案：

         - 若為AWS，請參閱 [新增和移除端點服務的許可權].
         - 若為Azure，請參閱 [管理私人端點連線]

      - 為Adobe提供客戶VPC的端點服務名稱。

   - **Adobe** 將客戶端點服務新增至Adobe平台帳戶(AWS或Azure)，這會觸發與客戶VPC的連線要求。
   - **客戶** 核准Adobe的連線要求以完成設定。
   - **客戶** [驗證連線](#test-vpc-endpoint-service-connection) 從AdobeVPC。
...
1. 從專案根目錄， **簽出** 設定為存取PrivateLink端點服務的測試或生產環境。
...
[提交Adobe Commerce支援票證](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/help-center-guide/magento-help-center-user-guide.html#submit-ticket) 變更現有的PrivateLink設定。 例如，您可以要求進行如下的變更：
...
- 一個 [介面端點] (AWS)或 [私用端點] (Azure)，可讓Adobe連線至您VPC上代管的端點服務
