...
- **專案管理員** — 寫入所有專案環境的存取權，並可管理使用者、推送程式碼和更新專案設定。
- **專案檢視器** — 所有專案環境的僅限檢視存取權。

專案檢視器無法在任何環境中執行任務；不過，您可以授予專案檢視器特定環境型別的寫入許可權。

環境層級的存取取決於環境型別：生產、測試和開發。 授與使用者 _檢視者_ 的許可權 _開發_ 環境表示他們可以檢視 **全部** 專案中的開發環境。 下表釐清授予各個許可權層級的功能：

| 許可權層級 | 存取 | SSH存取 |
| ------------------ | ----------- | :----------: |
| **管理員** | 執行管理員工作，例如變更設定、推播程式碼、執行工作及分支管理，包括合併父環境 | 是 |
| **投稿人** | 推送程式碼並分支環境；無法變更設定或執行動作 | 是 |
| **檢視者** | 對環境型別的僅限檢視存取權 | 否 |
| **無存取權** | 無法存取環境型別 | 否 |

{style="table-layout:auto"}

您可以使用新增使用者並指派角色 `magento-cloud` CLI或 [!DNL Cloud Console].

>[!BEGINSHADEBOX]

**先決條件：**

- 已註冊Adobe ID的使用者。 使用者必須 [註冊Adobe帳戶](https://account.adobe.com) 然後 [初始化他們的雲端帳戶](https://console.adobecommerce.com) 之後才能將其新增至雲端專案。
- 使用者已指派 **管理員** 角色無法透過管理使用者 `magento-cloud` CLI 僅限被授與的使用者 **帳戶擁有者** 角色可以管理使用者。

>[!ENDSHADEBOX]

## 使用CLI管理使用者

使用 `magento-cloud` CLI可管理使用者並與自動化系統整合：

- `magento-cloud user:add` — 新增使用者至專案
- `magento-cloud user:delete` — 刪除使用者
- `magento-cloud user:list [users]`-list專案使用者
- `magento-cloud user:role` — 檢視或變更使用者角色
- `magento-cloud user:update` — 更新專案上的使用者角色

下列範例使用 `magento-cloud` CLI可新增使用者、設定角色、修改專案指派及指派使用者角色。

**若要新增使用者並指派角色**：

1. 使用 `magento-cloud` 用於新增使用者的CLI。
   ...
   >[!IMPORTANT]
   >
   >使用者必須有Adobe ID；請參閱 [必備條件](#add-users-and-manage-access).
...
若要將使用者新增為 `viewer` 在 `Production` 環境，以及as a `contributor` 在 `Integration` 環境：
...
若要將使用者環境許可權更新至 `admin` 於 `Production` 環境：

```bash
magento-cloud user:update alice@example.com -r production:a
```

## 從管理使用者 [!DNL Cloud Console]

您可以使用 [[!DNL Cloud Console]](../../get-started/cloud-console.md) 以新增許可權並使用 _編輯_ 修改現有使用者許可權的功能。
...
1. 在 _專案設定_，按一下 **[!UICONTROL Access]**.

1. 在 _存取_ 檢視，按一下 **[!UICONTROL Add]**.
...
1. 完成 _[!UICONTROL Add User]_表單：
   ...
   - **[!UICONTROL Environment types and permissions]** — 授予特定環境型別的存取權和特定許可權等級。 _無存取權_， _管理員_ （變更設定、執行動作、合併程式碼）、 _投稿人_ （推播程式碼），或 _檢視者_ （僅供檢視）。

   >[!TIP]
   >
   >僅a **專案管理員** 可以在任何環境中管理使用者。 若要授與使用者對的存取權 **存取** 標籤，另一個 **專案管理員** 或 **帳戶擁有者** 必須指派該使用者 **專案管理員** 角色。
...
為了增加安全性，Adobe提供專案層級的多重驗證(MFA)強制要求，以要求對雲端基礎結構專案原始碼和環境上的Adobe Commerce進行SSH存取的雙重驗證(TFA)。 另請參閱 [為SSH啟用MFA](multi-factor-authentication.md).
...
- **啟用TFA** — 設定雙因素驗證，以符合安全性與法規遵循標準。 專案設定方式 [MFA強制執行](multi-factor-authentication.md) 使用SSH存取專案的帳戶需要TFA。

- **啟用SSH金鑰** — 需要存取雲端基礎結構原始程式碼存放庫上Adobe Commerce的使用者必須在他們的帳戶上啟用SSH金鑰。 另請參閱 [安全連線](../development/secure-connections.md).
...
有關安裝驗證器應用程式和啟用TFA的說明，請參閱 _帳戶設定_ 中的頁面 [!DNL Cloud Console].
...
1. 在 _安全性_ 標籤，按一下 **[!UICONTROL Set up application]**.
...
1. 將雲端基礎結構帳戶上的Adobe Commerce新增到驗證器應用程式。
   ...
   - 在 [!UICONTROL **[!UICONTROL TFA set up - Application]**] 頁面，在的「 」中輸入行動裝置的TFA代碼 **[!UICONTROL Application verification code]** 欄位。

   - 按一下 **[!UICONTROL Verify and save]**.

     如果代碼有效，Adobe會傳送通知至帳戶電子郵件地址，確認帳戶現在具有TFA。

1. 選填。 啟用 _信任的瀏覽器_ 在瀏覽器中快取驗證代碼30天的設定。
...
1. 儲存復原始碼。
   ...
   - 在 _TFA設定 — 復原_ 程式碼頁面，複製並儲存復原程式碼，以便在無法存取行動裝置或驗證應用程式時，可登入雲端基礎結構專案的Adobe Commerce。

   - 將復原始碼複製到其他位置，或寫下代碼，以防您失去對裝置或驗證應用程式的存取權。

   - 按一下 **儲存** 將程式碼儲存至您的帳戶，以便從您的帳戶安全性設定檢視和管理程式碼。
...
1. 完成TFA設定後，請按一下 **儲存** 以更新您的帳戶。

1. 使用TFA驗證您目前的工作階段。

   - 登出您的帳戶。
   - 使用您的使用者名稱和密碼登入。
   - 出現提示時，請輸入 `accounts.magento.cloud` 從行動裝置上的驗證器應用程式進入。

### 管理TFA設定和復原程式碼

您可以從以下網址管理雲端基礎結構帳戶上Adobe Commerce的TFA設定： _安全性_ 區段於 _我的設定檔_ 頁面。
...
1. 在 _我的設定檔_ 頁面，按一下 **[!UICONTROL API tokens]** 標籤。

1. 按一下 **[!UICONTROL Create API token]** 並輸入名稱，例如，指定符合機器使用者或使用API權杖的自動化流程的名稱。
